...
- **Model Selection**: Only declare `model` when the prompt has a *technical dependency* on specific model capabilities (e.g., requires extended context windows, specific API features, or validated model-specific behavior). Otherwise, omit the field to allow users to select their preferred model based on cost, performance, and capability preferences.
- Examples where `model` declaration **is appropriate**:
  - Prompts requiring >100K token context windows
  - Prompts validated against specific model API features
  - Prompts with model-specific fine-tuning or behavior dependencies
- Examples where `model` declaration **should be omitted**:
  - General code generation or analysis tasks
